...
export default class Page extends PureComponent {
  ...
  render() {
    const { cs } = this.props
    return (
      <div className={`${cp}-page`} onClick={() => this.setState({ showSizeOption: false })}>
        {this.createPage()}
      </div>
    )
  }
  createPage() {
    ...
    const { total, showTotal = true, showSizeChange, showJump, type } = this.props
  ...
  createPageSizeChange() {
    ...
    return (
      ...
      >
        ...
        <div className="pageSize-option" key="pageSize-option" onMouseOut={() => this.setState({ showSizeOption: false })}>
          {
            sizeAry.map((v, i) => (
              <div
                className={cls("pageSize-option-select", { on: pageSize == v })} key={i}
  ...
  createJump() {
    return (
      <div className="jump" key="jump">
        <span>跳转</span>
        <input type="text" onKeyPress={this.jumpPage} />
      </div>
    )
  }
  ...
  pagesizeChange = async (page, e) => {
    ...
    let Cpage = Math.ceil((current - 1) * pageSize / page)
    if (onChange) {
  ...
  createPageItem(i) {
    ...
    return (
      <div className={cls("page-item", { on: current === i })} key={i} onClick={() => {this.pageChange(i)}}>
        {
          pageFormat
            ? pageFormat.replace(/{.*}/g, i)
            : <span className="number" key={`number${i}`}>{i}</span>
        }
      </div>
    )
  }
  createPrevButton() {
    const { current } = this.state
    return (
      <div className={cls("page-item", { disabled: current <= 1 })} key="prev">
        <span
  ...
  createNextButton() {
    ...
    const { total } = this.props
    return (
      <div className={cls("page-item", { disabled: current >= total })} key="next">
        <span
  ...
  createPrev5Button() {
    const { current } = this.state
    return (
      <div className="page-item" key="leftover-prev">
        <span
  ...
  createNext5Button() {
    ...
    return (
      <div className="page-item" key="leftover-next">
        <span
  ...
  pageChange(i) {
    ...
    if (onChange) {
      ...
      let currentDataLast = pageSize * i
      this.setState({
        current: i
      })
    }
