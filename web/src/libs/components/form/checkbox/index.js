...
export default class HYCheckbox extends PureComponent {
  ...
  render() {
    const { cs } = this.props
    return (
      <div className={cls(`${cp}-checkbox`, cs)}>
        {this.createItems()}
      </div>
    )
  }
  createItems() {
    ...
    return dataSource.map((v, i) => (
      <div className={cls(`${cp}-checkbox-item`, disabled ? { disabled: disabled(v) } : {})} key={i}>
        {
          this.createItem(v, i)
        }
      </div>
  ...
  createItem(data, index) {
    ...
    case "box":
      return (
        ...
        >{data[label] || data[field] || data}</div>
      )
    default:
      return (
        <div className={`${cp}-checkbox-default`}>
          <div
