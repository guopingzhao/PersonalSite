...
export default class Table extends PureComponent {
  ...
  createHead() {
    const { column = [], multiselect, title, isHeadChecked, showThTitle, headCheckedKeys = this.state.headSelect, serial } = this.props
    let first = []
    if (multiselect) {
      const { selectRow, dataSource: { length } } = this.state
      const sellen = Object.keys(selectRow).length
      first.push(
        <th className={`${cp}-table-th`} key="th">
          <div
            className={cls("wrap", {
    ...
    if (serial) {
      ...
      first.push(
        <th className={cls(`${cp}-table-th`, { select: isHeadChecked })} key="serial">
          {
            isHeadChecked && <div
    ...
    let head = (column || []).map((v, i) => {
      ...
      return (<th className={cls(`${cp}-table-th`, { select: isHeadChecked })} key={v.key || i}>
        {
          isHeadChecked && <div
            className={cls("head-checked", {
              "whole-select": headCheckedKeys[val]
            })}
            onClick={() => this.selectHead(v)}
          />
        }
        <Tooltip  placement="right">
          <span alt={showThTitle && showThTitle(v)} >
            {val}
          </span>
        </Tooltip>
      </th>)
  ...
  createRow(data, i) {
    ...
    if (multiselect) {
      ...
      first.push(
        <td className={`${cp}-table-td`} key="td">
          <div className={cls("wrap", { "whole-select": selectRow[i] })} onClick={() => this.onSelect(data, i)}/>
        </td>
      )
    }
    if (serial) {
    ...
    let row = column.map((v, i) => {
      let { key, dataIndex, render } = v
      return (<td className={`${cp}-table-td`} key={v.key || dataIndex || i}>
        <span>{render ? render.call(this, data) : `${data[dataIndex] || data[i]}`}</span>
      </td>)
    })
